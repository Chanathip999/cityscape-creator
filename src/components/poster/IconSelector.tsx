...
export const IconSelector = ({ icons, onIconsChange, mapCenter }: IconSelectorProps) => {
  ...
  const updateIconSize = (iconId: string, newSize: number) => {
    onIconsChange(icons.map(icon => 
      icon.id === iconId ? { ...icon, size: newSize } : icon
    ));
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground w-12">Größe:</span>
          <Slider
            value={[iconSize * 100]}
            min={50}
            max={200}
            step={10}
            onValueChange={([v]) => setIconSize(v / 100)}
            className="flex-1"
          />
      ...
      {/* Active icons list with size control */}
      {icons.length > 0 && (
        <div className="space-y-1.5">
          {icons.map((icon) => {
            ...
            return (
              <div
                key={icon.id}
                className="flex items-center gap-2 py-1.5 px-2 rounded bg-muted/50 text-xs"
              >
                ...
                <Slider
                  value={[(icon.size || 1) * 100]}
                  min={50}
                  max={200}
                  step={10}
                  onValueChange={([v]) => updateIconSize(icon.id, v / 100)}

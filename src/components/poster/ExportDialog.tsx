...
import { ExportFormat, ExportResolution, PosterConfig, ASPECT_RATIOS } from '@/types/poster';
...
const PRICING_OPTIONS: { id: PricingTier; name: string; description: string; price: number; multiplier: number; hasWatermark: boolean }[] = [
  { id: 'budget', name: 'Budget', description: 'With Logo', price: 3.00, multiplier: 1, hasWatermark: true },
...
const BUNDLE_PRICING: Record<Exclude<PricingTier, 'budget'>, number> = {
  fullhd: 24.97, '4k': 39.97, '8k': 49.97,
};
...
export const ExportDialog = ({ config, posterRef }: ExportDialogProps) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      toast({
        title: t('payment.generating'),
        description: isBudgetTier ? t('payment.budgetGenerating') : t('payment.pleaseWait'),
      });
      ...
      if (format === 'png') {
        ...
        await new Promise<void>((resolve, reject) => {
          img.onload = () => {
            ...
            ctx.fillStyle = 'white'; ctx.fillRect(0, 0, exportWidth, exportHeight);
            ctx.drawImage(img, 0, 0);
            canvas.toBlob((blob) => {
              if (blob) { downloadBlob(blob, `${fileName}.jpg`); resolve(); }
      ...
      toast({
        ...
        description: `${config.city} ${t('export.savedAs')} ${format.toUpperCase()}.`,
      });
      setOpen(false);
    } catch (error) {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : t('export.tryAgain'),
        variant: 'destructive',
      });
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <Label className="text-base font-medium">{t('export.fileFormat')}</Label>
            <RadioGroup value={format} onValueChange={(value) => setFormat(value as ExportFormat)} className="grid grid-cols-2 gap-4">
              <div>
                <RadioGroupItem value="png" id="format-png" className="peer sr-only" />
                <Label htmlFor="format-png" className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary cursor-pointer">
                  <FileImage className="mb-3 h-6 w-6" />
                  <span className="font-medium">PNG</span>
                  <span className="text-xs text-muted-foreground">{t('export.lossless')}</span>
                </Label>
              </div>
              <div>
                <RadioGroupItem value="jpeg" id="format-jpeg" className="peer sr-only" />
                <Label htmlFor="format-jpeg" className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary cursor-pointer">
                  <Image className="mb-3 h-6 w-6" />
                  <span className="font-medium">JPEG</span>
                  <span className="text-xs text-muted-foreground">{t('export.compressed')}</span>
                </Label>
              </div>
            </RadioGroup>
          </div>
          ...
          {/* Pricing Tier */}
          <div className="space-y-3">
            <Label className="text-base font-medium">{t('export.pricingTier')}</Label>
            <RadioGroup value={pricingTier} onValueChange={(value) => { setPricingTier(value as PricingTier); if (value === 'budget') setQuantity(1); }} className="grid grid-cols-2 gap-2">
              {PRICING_OPTIONS.map((tier) => (
                <div key={tier.id}>
                  <RadioGroupItem value={tier.id} id={`tier-${tier.id}`} className="peer sr-only" />
                  <Label htmlFor={`tier-${tier.id}`} className={`flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-2.5 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary cursor-pointer relative ${tier.hasWatermark ? 'bg-muted/30' : ''}`}>
                    {tier.hasWatermark && (
          ...
          {isBudgetTier && (
            <div className="text-xs text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 rounded-lg p-2 flex items-start gap-2">
              ...
              <span>{t('export.budgetNotice')}</span>
            </div>
          )}

          {!isBudgetTier && (
            <div className="space-y-3">
              <Label className="text-base font-medium">{t('export.quantity')}</Label>
              <RadioGroup value={String(quantity)} onValueChange={(value) => setQuantity(Number(value) as 1 | 3)} className="grid grid-cols-2 gap-3">
                <div>
                  <RadioGroupItem value="1" id="qty-1" className="peer sr-only" />
                  <Label htmlFor="qty-1" className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-3 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary cursor-pointer">
                    <span className="font-medium">1 {t('export.poster')}</span>
                ...
                <div>
                  ...
                  <Label htmlFor="qty-3" className="flex flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-3 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary cursor-pointer relative">
                    <span className="absolute -top-2 -right-2 bg-primary text-primary-foreground text-[10px] px-1.5 py-0.5 rounded-full font-bold">{t('export.save')}</span>
                    <span className="font-medium">3 {t('export.posters')}</span>
                    <span className="text-sm font-bold text-primary mt-1">€{BUNDLE_PRICING[pricingTier as Exclude<PricingTier, 'budget'>]?.toFixed(2) || '-'}</span>
                    <span className="text-[10px] text-muted-foreground line-through">€{(currentPricing.price * 3).toFixed(2)}</span>
                  </Label>
                </div>
              </RadioGroup>
            </div>
          ...
          <Button onClick={handleDownload} disabled={isLoading} className="w-full gap-2" size="lg">
            {isLoading ? (
              <><Loader2 className="w-4 h-4 animate-spin" />{t('export.generating')}</>
            ) : (
              <><Download className="w-4 h-4" />{t('export.download')} {isBudgetTier ? '' : `${quantity}x `}{format.toUpperCase()} ({currentPricing.name}) – €{totalPrice.toFixed(2)}</>
            )}
          </Button>
        </div>
      </DialogContent>
